...
echo off 
 ...
 echo if ( !$( this ).parent().hasClass( "note" ) ) { >>###GRIBS.html 
 echo  console.log('HASNOCLASS'); >>###GRIBS.html 
 echo  $(this).wrap("<a class='note' target="+"'_blank'"+" href='" +$(this).attr("src").replace( ".jpg",'.pdf') + "'>"); >>###GRIBS.html 
 echo  } else { >>###GRIBS.html 
 echo  console.log('HASCLASS'); >>###GRIBS.html 
 echo } >>###GRIBS.html 
 echo })  >>###GRIBS.html 
...
for %%a in (*.jpg) do  Echo %%a | findstr /C:"%name%">nul && (echo  ^<a href="%%~na.pdf" target="_blank" ^>  ^<img  src="VIGNETTES/%%a" height="25%%" title="%%~na" /^> >> ###GRIBS.html ) || (echo  off)
